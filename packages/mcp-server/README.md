...
MCP Server for WechatSync - 连接 Codex / Claude Code 和 Chrome Extension，实现 AI 辅助文章同步。

## 架构

```
┌─────────────┐      stdio       ┌─────────────────┐     WebSocket     ┌─────────────┐
│ Codex/Claude│ <──────────────> │ MCP Server      │ <───────────────> │  Extension  │
│             │                  │ (Node.js)       │                   │ (Background)│
└─────────────┘                  └─────────────────┘                   └─────────────┘
```
...
## 传输方式选择（stdio vs SSE）
- **stdio（推荐）**：本地运行、无需端口、稳定，适合 Codex/Claude 桌面客户端。
- **SSE**：适合远程或多客户端共享（需要常驻 HTTP 服务）。
...
- `MCP_TOKEN` / `WECHATSYNC_TOKEN`: 安全验证 Token（必需，二选一）
