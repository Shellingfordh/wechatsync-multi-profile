...
export function SettingsDrawer({ open, onClose }: SettingsDrawerProps) {
  ...
  const [mcpServerUrl, setMcpServerUrl] = useState('ws://localhost:9527')
  ...
  useEffect(() => {
    ...
    chrome.runtime.sendMessage({ type: 'MCP_STATUS' }, (response) => {
      if (response && !response.error) {
        ...
        setMcpServerUrl(response.serverUrl || 'ws://localhost:9527')
      }
    })
                    ...
                    <button
                      ...
                      onClick={() => {
                        chrome.runtime.sendMessage(
                          { type: 'MCP_SET_SERVER_URL', payload: { serverUrl: mcpServerUrl } },
                          (response) => {
                            if (response?.success) {
                              setMcpStatus(prev => ({ ...prev, serverUrl: response.serverUrl }))
                            }
                          }
                        )
                      }}
