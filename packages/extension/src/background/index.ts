...
type MessageAction =
  ...
  | { type: 'MCP_DISABLE' }
  | { type: 'MCP_STATUS' }
  | { type: 'MCP_SET_SERVER_URL'; payload: { serverUrl: string } }
...
async function handleMessage(message: MessageAction, sender?: chrome.runtime.MessageSender) {
  switch (message.type) {
    ...
    case 'MCP_ENABLE': {
      ...
      // 设置 server URL（默认 ws://localhost:9527）
      const serverUrl = storage.mcpServerUrl || 'ws://localhost:9527'
    ...
    case 'MCP_STATUS': {
      ...
      return {
        ...
        serverUrl: storage.mcpServerUrl || 'ws://localhost:9527',
      }
    }

    case 'MCP_SET_SERVER_URL': {
      const serverUrl = message.payload.serverUrl?.trim() || 'ws://localhost:9527'
