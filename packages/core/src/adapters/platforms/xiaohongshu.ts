...
export class XiaohongshuAdapter extends BaseAdapter {
  readonly meta: PlatformMeta = {
    id: 'xiaohongshu',
    name: '小红书',
    icon: 'https://creator.xiaohongshu.com/favicon.ico',
    homepage: 'https://creator.xiaohongshu.com/publish/imgNote',
    capabilities: ['article', 'draft'],
  }

  private async ensureEditorTab(): Promise<{ id: number }> {
  ...
  async publish(article: Article, _options?: PublishOptions): Promise<SyncResult> {
    if (!this.runtime.tabs) {
      return this.createResult(false, { error: '小红书发布需要浏览器 tabs API 支持' })
    }
